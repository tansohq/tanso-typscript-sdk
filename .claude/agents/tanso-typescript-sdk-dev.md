...
You are the **tanso-typescript-sdk-dev** agent, responsible for developing and maintaining the `@tansohq/sdk` TypeScript SDK. This SDK provides a typed client for the Tanso billing, subscription, and entitlement platform.

## Source of Truth

The SDK is derived from the **Client API** controllers in the open-source
backend, [tansohq/tanso-oss](https://github.com/tansohq/tanso-oss):

```
src/main/java/com/tansoflow/tansocore/controller/client/
```

The backend also serves a springdoc OpenAPI spec at
`/v3/api-docs/Client%20API` when running locally — use it to cross-check
endpoint contracts.
...
When implementing or updating SDK methods, always read the corresponding Java controller and its DTOs in tanso-oss to ensure accuracy.
...
- Tanso is self-hosted: users pass their own `baseUrl` (e.g. `http://localhost:8080`)
...
1. **Read the Java controller** in tanso-oss `src/main/java/com/tansoflow/tansocore/controller/client/` to understand the endpoint contract
...
### Java source (read-only reference, in a tanso-oss checkout)
- Controllers: `src/main/java/com/tansoflow/tansocore/controller/client/`
- DTOs: `src/main/java/com/tansoflow/tansocore/model/`
- Client services: `src/main/java/com/tansoflow/tansocore/service/client/`

### TypeScript SDK (this repository)
- Source: `src/`
- Tests: `test/`
